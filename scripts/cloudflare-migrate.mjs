...
export function verifyEditorialProductionRows(rows, fixtures) {
  ...
  if (!Array.isArray(fixtures) || fixtures.length !== 10) {
    throw new Error(`Expected exactly 10 frozen editorial fixtures, found ${Array.isArray(fixtures) ? fixtures.length : 0}.`);
  }
  if (rows.length !== 10) throw new Error(`Expected exactly 10 current editorial heads in production, found ${rows.length}.`);
...
export async function migrateCloudflareProductionD1(env = process.env) {
  ...
  console.log(`Cloudflare D1 ready: ${appliedMigrationNames.length}/${localMigrationNames.length} migrations and P4-03C2 editorial current heads verified.`);
...
async function ensureEditorialBootstrap(configPath) {
  ...
  if (fixtures.length !== 10) throw new Error(`Expected exactly 10 editorial bootstrap fixtures, found ${fixtures.length}.`);

  const stagingRoot = await mkdtemp(path.join(path.dirname(configPath), "editorial-bootstrap-"));
  const stagedPath = path.join(stagingRoot, "p4-03-c2-editorial-bootstrap.sql");
  try {
    await writeFile(stagedPath, buildEditorialBootstrapSql(fixtures), { encoding: "utf8", mode: 0o600 });
    console.log("Applying idempotent P4-03C2 editorial bootstrap.");
    await runWrangler([
      "d1",
      "execute",
      "DB",
  ...
  console.log("Verified 10 production editorial current heads exactly match the frozen P4-03C2 bootstrap fixtures.");
}
