...
interface DirectoryRow extends CatalogRow { hasVerifiedReview: number; }

export type { PublicCatalogDirectoryReviewStatus } from "@/db/public-catalog-query";
...
export interface PublicCatalogDirectoryItem extends PublicCatalogTitle { hasVerifiedReview: boolean; }
...
export async function loadPublicCatalogTitle(qidInput: unknown): Promise<PublicCatalogTitle | null> {
  ...
  const row = await requireD1()
    ...
    .bind(publicCatalogTitleIdFromQid(qid), qid)
    .first<CatalogRow>();
...
export async function listPublicCatalogDirectory(input: PublicCatalogDirectoryInput): Promise<PublicCatalogDirectoryPage> {
  ...
  const plan = buildPublicCatalogDirectoryQueries({ query, kind, year, reviewStatus, limit: pageSize, offset });
  ...
  for (const row of result.results ?? []) {
    ...
    if (parsed) items.push({ ...parsed, hasVerifiedReview: row.hasVerifiedReview === 1 });
  }
