...
export async function searchPublicTitles(input: unknown): Promise<PublicTitleSearchResult[]> {
  ...
  const result = await requireD1().prepare(candidateQuery.sql).bind(...candidateQuery.bindings).all<CandidateRow>();
...
function parseCandidateRow(row: CandidateRow): PublicTitleSearchCandidate | null {
  ...
  if (row.originalName !== null && (typeof row.originalName !== "string" || !row.originalName.trim())) return null;
...
function parseTitleKind(value: string): PublicTitleKind | null {
  if (value === "movie" || value === "series" || value === "episode" || value === "special") return value;
  return null;
}
