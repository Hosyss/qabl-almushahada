...
test("bootstrap loads exactly ten editorial publications and is idempotent", async () => {
  ...
  assert.equal(db.prepare("SELECT COUNT(*) AS count FROM editorial_publication_heads").get().count, 10);
  assert.equal(db.prepare("SELECT COUNT(*) AS count FROM editorial_publication_revisions").get().count, 10);
