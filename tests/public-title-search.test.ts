...
const reviewState = { hasVerifiedReview: false, hasReviewInProgress: false } as const;
...
test("exact canonical match outranks a verified prefix match", () => {
  ...
  const results = rankPublicTitleSearchCandidates(parsed, [
    { id: "verified-prefix", canonicalName: "نيمو يعود", originalName: null, kind: "movie", releaseYear: 2026, hasVerifiedReview: true, hasReviewInProgress: false },
    { id: "exact", canonicalName: "نيمو", originalName: "Nemo", kind: "movie", releaseYear: 2003, ...reviewState },
  ]);
...
test("original-name exact match works for English searches", () => {
  ...
  const results = rankPublicTitleSearchCandidates(parsed, [
    { id: "nemo", canonicalName: "البحث عن نيمو", originalName: "Finding Nemo", kind: "movie", releaseYear: 2003, hasVerifiedReview: true, hasReviewInProgress: false },
  ]);
...
test("token matching can span canonical and original names without inventing fuzzy similarity", () => {
  ...
  const results = rankPublicTitleSearchCandidates(parsed, [
    { id: "nemo", canonicalName: "البحث عن نيمو", originalName: "Finding Nemo", kind: "movie", releaseYear: 2003, hasVerifiedReview: true, hasReviewInProgress: false },
    { id: "other", canonicalName: "رحلة بحرية", originalName: "Finding Dory", kind: "movie", releaseYear: 2016, hasVerifiedReview: true, hasReviewInProgress: false },
  ]);
...
test("public ranking is deterministically capped", () => {
  ...
  const candidates = Array.from({ length: 20 }, (_, index) => ({
    id: `title-${index}`, canonicalName: `فيلم ${index}`, originalName: null, kind: "movie" as const,
    releaseYear: 2000 + index, ...reviewState,
  }));
  assert.equal(rankPublicTitleSearchCandidates(parsed, candidates).length, MAX_PUBLIC_TITLE_SEARCH_RESULTS);
});
