...
import { listPublicCatalogDirectory, type PublicCatalogDirectoryReviewStatus } from "@/db/public-catalog-service";
...
type Params = { q?: string | string[]; kind?: string | string[]; year?: string | string[]; reviewStatus?: string | string[]; page?: string | string[] };
...
export default async function TitlesPage({ searchParams }: Props) {
  ...
  try { data = await listPublicCatalogDirectory({ query, kind, year, reviewStatus, page, pageSize: 24 }); } catch { data = null; }
...
function pageHref(params: Params, page: number) {
  const search = new URLSearchParams(); const q = one(params.q).trim().slice(0, 80); const kind = parseKind(one(params.kind)); const year = parseYear(one(params.year)); const reviewStatus = parseReviewStatus(one(params.reviewStatus));
  if (q) search.set("q", q); if (kind !== "all") search.set("kind", kind); if (year !== null) search.set("year", String(year)); if (reviewStatus !== "all") search.set("reviewStatus", reviewStatus); if (page > 1) search.set("page", String(page));
