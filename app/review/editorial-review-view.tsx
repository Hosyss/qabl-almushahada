...
export default async function EditorialReviewView({ review }: { review: EditorialReviewPublication }) {
  ...
  const claimsByCategory = new Map(
    CONTENT_CATEGORIES.map((category) => [category, review.claims.filter((claim) => claim.category === category)]),
  );
  ...
  const corroboratedFacts = review.claims.filter((claim) => claim.verification === "corroborated").map((claim) => claim.summaryAr);
  const uncertainLabels = review.uncertainCategories.map((category) => getEditorialCategoryLabelAr(category));
  const structuredData = JSON.stringify({
    "@context": "https://schema.org",
    "@type": "Article",
    headline: `${presentation.titleAr} — ${presentation.titleEn} (${review.releaseYear}) — تحليل محتوى موثق جزئيًا`,
      ...
      <section className="review-title-card" aria-labelledby="review-title">
        ...
        <div className="review-title-copy">
          ...
          <div className="review-version-line">
            ...
            <span>revision {presentation.revision}</span>
            <span>نُشر: {formatDate(review.publishedAt)}</span>
            <span>آخر تحديث: {formatDate(presentation.updatedAt)}</span>
          </div>
        </div>
        <div className="review-verification">
          <span className="review-verification__icon" aria-hidden="true">◎</span>
          <span><small>حالة الحكم</small><strong>المعلومات غير كافية لإصدار حكم نهائي</strong></span>
        </div>
      </section>
      ...
      <div className="review-layout">
        <div className="review-content">
          <section className="review-decision" aria-labelledby="decision-title">
            ...
            <div>
              ...
              <h2 id="decision-title">المعلومات غير كافية لإصدار حكم نهائي</h2>
              <p>
          ...
          <section className="editorial-primary-summary" aria-labelledby="editorial-primary-summary-title">
            <div>
              ...
              <h2 id="editorial-primary-summary-title">الخلاصة هي نقطة البداية، وليست ملاحظة مدفونة.</h2>
            ...
            <EditorialSummaryDialog
              ...
              corroboratedFacts={corroboratedFacts}
              uncertainLabels={uncertainLabels}
              sources={review.sources.map((source) => ({ publisher: source.publisher, sourceUrl: source.sourceUrl, rightsLabel: source.rightsLabel }))}
          ...
          <section className="review-logic" aria-labelledby="analysis-title">
            ...
            <div className="review-transparency">
              ...
              <div><strong>النص العربي مكتوب من الصفر، ولا ينقل صياغة مراجعة خارجية أو تقييمها العددي.</strong><p>{review.analysisAr}</p></div>
            </div>
          ...
          <section className="review-breakdown" aria-labelledby="breakdown-title">
            <div className="review-section-head"><div><span>المحتوى بالتفصيل</span><h2 id="breakdown-title">إيه اللي نقدر نثبته حاليًا؟</h2></div></div>
                ...
                return (
                  <article className="review-category is-open" key={category}>
                    <div className="review-category__facts" id={`editorial-category-${category}`}>
                      <div className="review-facts-label"><span>{getEditorialCategoryLabelAr(category)}</span><small>{isUncertain ? "غير محسوم" : "توجد واقعة/وقائع مدعومة"}</small></div>
                      {isUncertain ? (
                        <div className="review-fact"><time>؟</time><p>المصادر الحالية لا تكفي لحسم هذا المحور. لا نعامل غياب الذكر أو تقييم مصدر واحد كدليل عام على عدم الوجود.</p></div>
                      ) : (
                        categoryClaims.map((claim) => {
                          const claimSources = claim.sourceIds.map((sourceId) => sourcesById.get(sourceId)).filter((source) => Boolean(source));
                          return (
                            <div className="review-fact editorial-fact" key={claim.id}>
                              <time>{claim.verification === "corroborated" ? "✓✓" : "✓"}</time>
                              <div>
                                <span className={`editorial-strength editorial-strength--${claim.verification}`}>{claim.verification === "corroborated" ? "اتفاق مستقل" : "مصدر واحد فقط"}</span>
                                <p>{claim.summaryAr}</p>
                                <small className="editorial-source-links">المصادر: {claimSources.map((source, index) => <span key={source!.id}>{index > 0 ? " · " : ""}<a href={`#${source!.id}`}>{source!.publisher}</a></span>)}</small>
                              </div>
                            </div>
                          );
                        })
          ...
          <section className="review-logic" aria-labelledby="method-title">
            <div className="review-section-head review-section-head--simple"><div><span>منهج P4-03 التحريري</span><h2 id="method-title">إزاي بنفصل المعلومة عن الحكم؟</h2></div></div>
            <div className="review-logic-steps">
              ...
              <article><b>3</b><div><strong>نترك المجهول مجهولًا</strong><p>المحور غير المحسوم لا يتحول إلى حالة غياب مؤكدة، وهذه الصفحة لا تملك سلطة إصدار حكم ملاءمة.</p></div></article>
            </div>
          </section>

          <section className="review-transparency"><span className="review-transparency__mark" aria-hidden="true">◎</span><div><strong>نطاق التحليل</strong><p>{review.scopeAr}</p></div></section>

          <section className="review-logic" aria-labelledby="sources-title">
            <div className="review-section-head review-section-head--simple"><div><span>شفافية المصدر والحقوق</span><h2 id="sources-title">المصادر، ما تدعمه، وأساس الاستخدام</h2></div></div>
              ...
              {review.sources.map((source, index) => (
                <article key={source.id} id={source.id}>
                  ...
                  <div>
                    ...
                    <p>يدعم: {source.supportedClaimIds.map((claimId) => claimsById.get(claimId)?.category).filter((category): category is NonNullable<typeof category> => Boolean(category)).map((category) => getEditorialCategoryLabelAr(category)).join("، ")}</p>
                    <p className="editorial-rights-note"><strong>أساس الاستخدام:</strong> {source.rightsLabel}</p>
                    <p>{source.usageNoteAr}</p>
                    <p className="editorial-source-actions"><a href={source.sourceUrl} target="_blank" rel="noreferrer">فتح المصدر</a><a href={source.rightsUrl} target="_blank" rel="noreferrer">الرخصة / الشروط</a></p>
                  </div>
                </article>
              ))}
            </div>
          ...
          <section className="editorial-ownership" aria-labelledby="editorial-ownership-title">
            <div><span>إثبات النسخة</span><h2 id="editorial-ownership-title">© قبل المشاهدة — تحليل تحريري أصلي</h2></div>
            <dl>
            ...
            <p>البصمة مرتبطة بمحتوى سجل النشر الحالي وتساعد على إثبات أي نسخة كانت منشورة في هذا revision.</p>
            <Link href="/corrections">الإبلاغ عن إعادة نشر المحتوى دون إذن</Link>
          </section>
        </div>
        ...
        <aside className="review-aside" aria-label="حالة التحليل التحريري">
          <section className="family-result-card">
            ...
            <div className="family-result-orb is-general"><span aria-hidden="true">!</span><small>حالة القرار</small><strong>المعلومات غير كافية لإصدار حكم نهائي</strong><p>الوقائع المنشورة تساعدك تفهم المحتوى، لكنها لا تكفي وحدها لإصدار «مناسب» أو «غير مناسب».</p></div>
          </section>
