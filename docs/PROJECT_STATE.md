...
## P3-01 — البحث العربي الحقيقي — مكتملة تقنيًا على الفرع / جاهزة للدمج
...
- CI #257 على الرأس `fe3cfb5ea975eff1e21c0d47e79ea7ed503e455e` نجح في **143/143 اختبارًا، 0 فشل**، ونجح `test:migrations`, `lint:local`, و`build:local` أيضًا.
- الخدمة جاهزة للبيانات الحقيقية، لكن الواجهة الرئيسية ما زالت تعرض رسالة البحث التجريبية؛ ربط نتائج البحث المرئية وحالاتها هو `P3-02` عمدًا حتى لا نخلط محرك البحث بواجهة النتائج.
- **لم تُدمج P3-01 على `main` بعد**؛ الحالة الحالية على الفرع `agent/p3-01-arabic-search` وتحتاج PR + CI مستقل + main CI قبل إعلانها مكتملة نهائيًا.
...
- واجهة البحث الرئيسية ما زالت تعرض الرسالة التجريبية؛ محرك P3-01 الحقيقي لم يُربط بصريًا بعد، وهذا هو نطاق `P3-02`.
...
1. بعد دمج `P3-01`: `P3-02` **خفيف / مجاني** — صفحة نتائج البحث وحالات: موجود، قيد المراجعة، غير موجود، وربط البحث المرئي بالخدمة الحقيقية.
