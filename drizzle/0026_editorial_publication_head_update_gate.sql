CREATE TRIGGER `editorial_publication_heads_update_gate`
BEFORE UPDATE ON `editorial_publication_heads`
BEGIN
  SELECT CASE WHEN NEW.`title_id` <> OLD.`title_id` OR NEW.`public_id` <> OLD.`public_id`
    THEN RAISE(ABORT, 'editorial head identity is immutable') END;
  SELECT CASE WHEN NEW.`revision` <> OLD.`revision` + 1
    THEN RAISE(ABORT, 'editorial head optimistic revision lock failed') END;
  SELECT CASE WHEN NEW.`last_transition_id` = OLD.`last_transition_id`
    THEN RAISE(ABORT, 'editorial head transition id must change') END;
  SELECT CASE WHEN NOT EXISTS (
    SELECT 1 FROM `editorial_publication_revisions` publication
    WHERE publication.`id` = NEW.`current_revision_id`
      AND publication.`title_id` = OLD.`title_id`
      AND publication.`public_id` = OLD.`public_id`
      AND publication.`revision` = NEW.`revision`
      AND publication.`revision_kind` = 'revision'
      AND publication.`supersedes_revision_id` = OLD.`current_revision_id`
      AND publication.`decision_status` = 'insufficient_data'
      AND publication.`decision_eligible` = 0
  ) THEN RAISE(ABORT, 'editorial head can move only to the direct next revision') END;
